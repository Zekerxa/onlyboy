          ...
          if (paramsArray.length) {
            ...
            var groupedParams = paramsArray.reduce(function (acc, key) {
              ...
              if (key.match(/^utm_/) || key.match(/^(d|g)clid/)) {
                var cleanName = key.replace(/^utm_/, '');
...
function identifyMiddleware(instance) {
  var storage$$1 = instance.storage;
  return function (store) {
    return function (next) {
      return function (action) {
        ...
        if (action.type === EVENTS.reset) {
          storage$$1.removeItem(USER_ID);
          storage$$1.removeItem(USER_TRAITS);
          storage$$1.removeItem(ANON_ID);
        ...
        if (action.type === EVENTS.identify) {
          var currentId = storage$$1.getItem(USER_ID);
          var currentTraits = storage$$1.getItem(USER_TRAITS) || {};
          ...
          if (userId) {
            storage$$1.setItem(USER_ID, userId);
          }
          /* Save user traits */


          if (traits) {
            storage$$1.setItem(USER_TRAITS, objectSpread2({}, currentTraits, {}, traits));
          }
        }
      ...
      while (1) {
        switch (_context5.prev = _context5.next) {
          case 0:
            ...
            method = action.type;
            isStartEvent = method.match(/Start$/); // console.log(`data ${method}`, data)
          ...
          case 13:
            ...
            !method.match(/^ready/) && !method.match(/^bootstrap/) && !method.match(/^params/) && !method.match(/^userIdChanged/))) {
              _context5.next = 21;
              break;
            }
...
function notAbortableError(action, method) {
  return function () {
    throw new Error("Action \"".concat(action.type, "\" is not cancellable. Remove abort in ").concat(method));
  };
}
/**
...
function getBrowserOS() {
  ...
  if (os.indexOf('Linux') !== -1) return 'Linux'; // default
...
function heartBeat(store, getPlugins, instance) {
  var timer = setInterval(function () {
    ...
    var _store$getState = store.getState(),
        ...
        user = _store$getState.user; // console.log('CURRENT Q', queue)
...
function enrich(payload) {
  ...
  var keys = ['userId', 'anonymousId'];
  return keys.reduce(function (acc, key) {
    var updated = fixEmptyValues(acc, user, key); // console.log('updated', updated)
...
function analytics() {
  ...
  var parsedOptions = (config.plugins || []).reduce(function (acc, p) {
    if (typeof p !== 'function' && p.NAMESPACE) {
      var NAMESPACE = p.NAMESPACE; // if plugin exposes EVENTS capture available events
      ...
      if (acc.plugins[NAMESPACE]) {
        throw new Error("\"".concat(NAMESPACE, "\" plugin loaded twice"));
      }
      ...
      if (!acc.plugins[NAMESPACE].loaded) {
        ...
        acc.plugins[NAMESPACE].loaded = function () {
          return true;
        };
      }
  ...
  var nonAbortable = function nonAbortable() {
    throw new Error('Abort not allowed in listener');
  };
  /**
    ...
    */
    ...
    dispatch: function dispatch(action) {
      ...
      if (isReservedAction(actionData.type)) {
        throw new Error("Trying to dispatch reservedAction \"".concat(actionData.type, "\""));
      }
